  ...
  return (
    <div style={{ position:'absolute', inset:0, zIndex:10, background:'var(--space)' }}>
      <div style={{ height:'100vh', overflowY:'auto', paddingBottom:72 }}>
        ...
        {/* Header */}
        <div className="animate-slide-up stagger-1" style={{ padding:'16px 20px 0', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div>
            <div style={{ fontSize:11, color:'rgba(255,255,255,0.4)', letterSpacing:2 }}>WELCOME BACK</div>
            <div style={{ fontFamily:'Orbitron,monospace', fontSize:15, fontWeight:700 }}>COMMANDER</div>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <div style={{
              ...
              fontFamily:'Orbitron,monospace', fontSize:13, fontWeight:700, color:'#000', cursor:'pointer',
            }}>RW</div>
          </div>
        </div>
        ...
        <div className="balance-card animate-slide-up stagger-2">
          ...
          <div style={{ fontFamily:'Orbitron,monospace', fontSize:32, fontWeight:900, color:'#fff', margin:'4px 0' }}>
          ...
          <div style={{ position:'absolute', right:20, top:20, fontSize:44, opacity:0.12 }}>🌌</div>
        </div>

        {/* Quick Actions */}
        <div className="animate-slide-up stagger-3" style={{ display:'flex', gap:8, margin:'0 16px 16px', justifyContent:'space-around' }}>
          {[
        ...
        <div style={{ padding:'0 16px', display:'flex', flexDirection:'column', gap:8 }}>
          {CRYPTOS.map((c, i) => (
            <div key={c.sym} className={`crypto-row animate-slide-up stagger-${Math.min(i+2,6)}`}
              onClick={() => openCoin(c)}>
              <div style={{
        ...
        <div style={{ padding:'0 16px', display:'flex', flexDirection:'column', gap:8 }}>
          {transactions.slice(0,4).map((tx) => (
            <div key={tx.id} style={{
              ...
              <div style={{ textAlign:'right' }}>
                <div style={{ fontFamily:'Orbitron,monospace', fontSize:12, color: tx.type==='receive' ? '#00ff88' : tx.type==='swap' ? '#9b59ff' : '#ff4466' }}>
